  ...
  <?php if (!empty($products)): ?>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      <?php foreach ($products as $p): ?>
        <div class="bg-white shadow hover:shadow-lg transition">
          ...
          <div class="p-4 space-y-1">
            <h2 class="font-semibold text-base"><?= htmlspecialchars($p['ProductName']) ?></h2>
            <p class="text-gray-500 text-sm"><?= htmlspecialchars($p['CategoryName']) ?></p>
            <p class="text-gray-800 font-bold">$<?= number_format($p['Price'], 2) ?></p>
          </div>
        </div>
      <?php endforeach; ?>
    </div>
  <?php else: ?>
    <p class="text-center text-gray-600">No products found in the Women’s category.</p>
  <?php endif; ?>
